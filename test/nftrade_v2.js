...
contract('NFTrade_v2', (accounts) => {
  ...
  describe('Trade', ()=>{
    ...
    it('cannot place offer against erc20', async ()=>{
      await truffleAssert.reverts(Trade.addOffer(NFT.address, 123, Token.address, 456, 1, fromUser1), 'Not allowed to make offers for erc20')
    })

    it('cannot place offer without approval', async ()=>{
      await truffleAssert.reverts(Trade.addOffer(NFT.address, 123, NFT.address, 456, 1, fromUser1), 'Handler unable to transfer NFT')
    })
    
    it('cannot offer un-owned token', async ()=>{
      await truffleAssert.reverts(Trade.addOffer(NFT.address, 456, NFT.address, 123, 1, fromUser1), 'Sender not owner of NFT')
    })
    ...
    it('cannot add erc20 token offer when canOfferERC20 off', async ()=>{
      ...
      await truffleAssert.fails(Trade.addOffer(Token.address, 0, NFT.address, 456, 1, fromUser1))
    })

    it('cannot add erc20 token offer before allowance', async ()=>{
      Trade.toggleCanOfferERC20(fromOwner)
      Token.mint(fromUser1.from, 10000000000000, fromOwner)
      await truffleAssert.fails(Trade.addOffer(Token.address, 0, NFT.address, 456, 1, fromUser1), 'Not Enough Allowance')
    })
    ...
    it('can add erc20 token offer after allowance', async ()=>{
      ...
      Trade.addOffer(Token.address, 0, NFT.address, 456, 1, fromUser1)
    })
    ...
    it('can reject offer', async ()=>{
      NFT.setApprovalForAll(Trade.address, true, fromUser1)
      await Trade.addOffer(NFT.address, 123, NFT.address, 456, 1, fromUser1)
      let offer = await Trade.getOffer(NFT.address, 456, 0)
      assert.equal(offer._from, fromUser1.from)
      assert.equal(offer.tokenId, 123)
      assert.equal(offer.token, NFT.address)
    ...
    it('cannot accept offer without approval', async ()=>{
      NFT.setApprovalForAll(Trade.address, true, fromUser1)
      Trade.addOffer(NFT.address, 123, NFT.address, 456, 1, fromUser1)
      await truffleAssert.reverts(Trade.acceptOffer(NFT.address, 456, 0, fromUser2), 'Handler unable to transfer NFT')
    })
    ...
    it('cannot accept erc20 offer without approval', async ()=>{
      ...
      Token.mint(fromUser1.from, 10000000000000, fromOwner)
      Token.approve(Trade.address, 100, fromUser1)
      await Trade.addOffer(Token.address, 0, NFT.address, 789, 1, fromUser1)
      await truffleAssert.reverts(Trade.acceptOffer(NFT.address, 789, 0, fromUser3), 'Handler unable to transfer NFT')
    })
    ...
    it('can accept erc20 offer with approval', async ()=>{
      ...
      Token.mint(fromUser1.from, 10000000000000, fromOwner)
      Token.approve(Trade.address, 100, fromUser1)
      Trade.addOffer(Token.address, 0, NFT.address, 789, 1, fromUser1)
      NFT.setApprovalForAll(Trade.address, true, fromUser3)
      await Trade.acceptOffer(NFT.address, 789, 0, fromUser3)
    ...
    it('cannot accept offer for un-owned nft', async ()=>{
      ...
      Trade.addOffer(NFT.address, 123, NFT.address, 456, 1, fromUser1)
      NFT.setApprovalForAll(Trade.address, true, fromUser3)
      await truffleAssert.fails(Trade.acceptOffer(NFT.address, 456, 0, fromUser3), 'Sender is not owner of NFT')
    })
    ...
    it('accepting offer removes all other offers', async ()=>{
      ...
      await Trade.addOffer(NFT.address, 123, NFT.address, 456, 1, fromUser1)
      await Trade.addOffer(NFT.address, 789, NFT.address, 456, 1, fromUser3)

      let count = await Trade.getOfferCount(NFT.address, 456)
      assert.equal(count, 2)

      await truffleAssert.passes(Trade.acceptOffer(NFT.address, 456, 1, fromUser2))
    ...
    it('can get outstanding offers placed', async ()=>{
      NFT.setApprovalForAll(Trade.address, true, fromUser1)
      Trade.addOffer(NFT.address, 123, NFT.address, 456, 1, fromUser1)
      Trade.addOffer(NFT.address, 123, NFT.address, 789, 1, fromUser1)
  ...
  describe('Payment', ()=>{
    ...
    beforeEach(async ()=>{
      ...
      Trade.changeOfferPrices(1000000000, 10000000000, 0, fromOwner)
    })
    ...
    describe('Pay to make offer', ()=>{
      it('fails to add offer if trade contract not approved to spend', async ()=>{
        await truffleAssert.reverts(Trade.addOffer(NFT.address, 123, NFT.address, 456, 1, fromUser1))
      })
      ...
      it('fails to add offer if too broke', async ()=>{
        ...
        await truffleAssert.reverts(Trade.addOffer(NFT.address, 123, NFT.address, 456, 1, fromUser1), 'Insufficient Balance for payment')
      })

      it('can make offer', async()=>{
        Token.approve(Trade.address, 10000000000, fromUser1)
        await truffleAssert.passes(Trade.addOffer(NFT.address, 123, NFT.address, 456, 1, fromUser1))
      })
    ...
    describe('Pay to accept offer', ()=>{
      ...
      it('fails to accept offer if too broke', async ()=>{
        ...
        await truffleAssert.reverts(Trade.acceptOffer(NFT.address, 456, 0, fromUser2), 'Insufficient Balance for payment')
      })

      it('can accept offer', async ()=>{
        Trade.changeOfferPrices(10, 100, 0, fromOwner)
  ...
  describe('Trade Types', ()=>{
    beforeEach(()=>{
      ...
      Trade.addOffer(NFT.address, 123, NFT2.address, 456, 1, fromUser1)
      Trade.addOffer(NFT2.address, 456, NFT3.address, 789, 1, fromUser2)
      Trade.addOffer(NFT3.address, 1337, NFT3.address, 789, 1, fromUser4)
      Trade.addOffer(Token.address, 0, NFT2.address, 456, 1, fromUser1)
      Trade.addOffer(Token.address, 0, NFT3.address, 789, 1, fromUser1)
    })
    ...
    it('can swap erc20 for erc721', async ()=>{
      ...
      await Trade.acceptOffer(NFT2.address, 456, 1, fromUser2)
    ...
    it('can swap erc20 for erc1155', async ()=>{
      ...
      await Trade.acceptOffer(NFT3.address, 789, 2, fromUser3)
  ...
  describe('Percentage', ()=>{
    ...
    describe('percentage fee for erc20 offer', async()=>{
      beforeEach(()=>{
        ...
        Trade.changeOfferPrices(0, 0, 10)
        ...
        Trade.addOffer(Token.address, 0, NFT.address, 456, 10, fromUser1)
      ...
      it('Percentage fees are paid when percentage fee for erc20 on', async ()=>{
        ...
        assert.equal((await Token.balanceOf(await Trade.recipientAddress())).toNumber(), 0)
        assert.equal(balanceOfErc20.toNumber(), 0)
        assert.equal(ownerOfERC721, fromUser2.from)
        ...
        assert.equal(ownerOfERC721, fromUser1.from)
        assert.equal((await Token.balanceOf(await Trade.recipientAddress())).toNumber(), 1)
      })
